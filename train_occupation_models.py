"""This model creates models for occupation code predictions"""
...
def train_best_model(df_train, target_layer, column_text, index):
    """Trains best model using hyperparameters from a file"""
    ...
    X_train_vectorized_stack = sparse.hstack(
        (
            X_train_vectorized,
            sparse.coo_matrix(X_train["F73"]).reshape(X_train.shape[0], 1),
        )
    )
    X_train_vectorized_stack = sparse.hstack(
        (
            X_train_vectorized_stack,
            sparse.coo_matrix(X_train["F72_2_N"]).reshape(X_train.shape[0], 1),
        )
    )

    print("Comienza la entrenamiento")
    model = SGDClassifier(
        alpha=clf__alpha, max_iter=300, penalty=clf__penalty, tol=1e-6
    )
    ...
    X_test_vectorized_stack = sparse.hstack(
        (
            X_test_vectorized,
            sparse.coo_matrix(X_test["F73"]).reshape(X_test.shape[0], 1),
        )
    )
    X_test_vectorized_stack = sparse.hstack(
        (
            X_test_vectorized_stack,
            sparse.coo_matrix(X_test["F72_2_N"]).reshape(X_test.shape[0], 1),
        )
    )
...
def train_base_layer_classifier(df):
    """Trains the base layer of occupation code"""
...
def train_first_layer_classifier(df):
    """Trains the first layer of occupation code"""
...
def train_second_layer_classifier(df):
    """Trains the second layer of occupation code"""
    path = ".\modelos_ocupacion\\"
    scores_path = ".\scores_ocupacion\\"
...
def train_third_layer_classifier(df):
    """Trains the third layer of occupation code"""
    path = ".\modelos_ocupacion\\"
    scores_path = ".\scores_ocupacion\\"
...
def train_ocupation_models():
    """Trains models than predict a code of occupation from a context"""
