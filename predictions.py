...
def occupation_base_layer_prediction(text,act_code,F73_code):
    ...
    scores = pd.read_excel('.\scores_ocupacion\\base_layer_scores.xlsx',engine='openpyxl')
...
def occupation_first_layer_prediction(text,act_code,F73_code,category):
    ...
    model_name = get_relative_path(".\modelos_ocupacion\\first_layer_{}_model.pickle".format(category))
    vect_name =  get_relative_path(".\modelos_ocupacion\\first_layer_{}_vect.pickle".format(category))
    if model_name in all_files:
        ...
        y_pred = model.predict(vect_text_stack)
        scores = pd.read_excel('.\scores_ocupacion\\first_layer_scores.xlsx',engine='openpyxl')
        scores.set_index(scores.columns[0],inplace=True)
...
def occupation_second_layer_prediction(text,act_code,F73_code,first_category,second_category):
    """Predicts a second layer coding for an occupation given a text and above categories"""
    models_folder = get_relative_path(".\modelos_ocupacion\\")
    all_files = glob(models_folder + "/*.pickle")
    model_name = get_relative_path(".\modelos_ocupacion\\first_layer_{}_second_layer_{}_model.pickle".format(first_category,second_category))
    vect_name =  get_relative_path(".\modelos_ocupacion\\first_layer_{}_second_layer_{}_vect.pickle".format(first_category,second_category))
    if model_name in all_files:
        model_f = open(model_name, "rb")
        model = pickle.load(model_f)
        model_f.close()
        ...
        y_pred = model.predict(vect_text_stack)
        scores = pd.read_excel('.\scores_ocupacion\\second_layer_scores.xlsx',engine='openpyxl')
        scores.set_index(scores.columns[0],inplace=True)
...
def occupation_third_layer_prediction(text,act_code,F73_code,first_category,second_category,third_category):
    """Predicts a third layer coding for an occupation given a text and above categories"""
    models_folder = get_relative_path(".\modelos_ocupacion\\")
    all_files = glob(models_folder + "/*.pickle")
    model_name = get_relative_path(".\modelos_ocupacion\\first_layer_{}_second_layer_{}_third_layer_{}_model.pickle".format(first_category,second_category,third_category))
    vect_name =  get_relative_path(".\modelos_ocupacion\\first_layer_{}_second_layer_{}_third_layer_{}_vect.pickle".format(first_category,second_category,third_category))
    if model_name in all_files:
        model_f = open(model_name, "rb")
        model = pickle.load(model_f)
        model_f.close()
        ...
        y_pred = model.predict(vect_text_stack)
        scores = pd.read_excel('.\scores_ocupacion\\third_layer_scores.xlsx',engine='openpyxl')
        scores.set_index(scores.columns[0],inplace=True)
        ...
        if "{}{}{}".format(first_category,second_category,third_category) in ['322','832']: #### MODIFICAR SI SE TRABAJA CON TODO EL DATASET
            last = 1
        else: 
...
def occupation_predict_coding(text,act_code,F73_code):
    ...
    first_layer_pred,  first_acc= occupation_first_layer_prediction(text,act_code,F73_code,base_cat_pred)    
    second_layer_pred, second_acc = occupation_second_layer_prediction(text,act_code,F73_code,base_cat_pred,first_layer_pred)
    third_layer_pred, third_acc = occupation_third_layer_prediction(text,act_code,F73_code,base_cat_pred,first_layer_pred,second_layer_pred)
    total_acc = base_acc * first_acc * second_acc * third_acc
    code = ("{}{}{}{}".format(base_cat_pred,first_layer_pred,second_layer_pred,third_layer_pred))
...
def activity_first_layer_prediction(text,first_category):
    ...
    all_files = glob(models_folder + "/*.pickle")
    model_name = get_relative_path(".\modelos_actividad\\first_layer_{}_model.pickle".format(first_category))
    vect_name =  get_relative_path(".\modelos_actividad\\first_layer_{}_vect.pickle".format(first_category))
...
def activity_second_layer_prediction(text,first_category,second_category):
    ...
    models_folder = get_relative_path(".\modelos_actividad\\")
    all_files = glob(models_folder + "/*.pickle")
    model_name = get_relative_path(".\modelos_actividad\\first_layer_{}_second_layer_{}_model.pickle".format(first_category,second_category))
    vect_name =  get_relative_path(".\modelos_actividad\\first_layer_{}_second_layer_{}_vect.pickle".format(first_category,second_category))
...
def activity_third_layer_prediction(text,first_category,second_category,third_category):
    """Predicts a third layer coding for an occupation given a text and above categories"""
    models_folder = get_relative_path(".\modelos_actividad\\")
    all_files = glob(models_folder + "/*.pickle")
    model_name = get_relative_path(".\modelos_actividad\\first_layer_{}_second_layer_{}_third_layer_{}_model.pickle".format(first_category,second_category,third_category))
    vect_name =  get_relative_path(".\modelos_actividad\\first_layer_{}_second_layer_{}_third_layer_{}_vect.pickle".format(first_category,second_category,third_category))
...
def activate_predict_coding(text):
    ...
    second_layer_pred, second_acc = activity_second_layer_prediction(text,base_cat_pred,first_layer_pred)
    third_layer_pred, third_acc = activity_third_layer_prediction(text,base_cat_pred,first_layer_pred,second_layer_pred)
    total_acc = base_acc * first_acc * second_acc * third_acc
    code = ("{}{}{}{}".format(base_cat_pred,first_layer_pred,second_layer_pred,third_layer_pred))
...
def get_vocab_ocu():
    vocab = []
    path_vocab_ocupation = '.\data\Vocabulario Ocupacion.txt'
    with open(path_vocab_ocupation,'r') as f:
        lines = f.readlines()
        for line in lines:
            word = line.strip().replace("'","")
            vocab.append(word)
...
def predict_code(text_ocupation, text_activity,F73_code,df_ocu=[],df_act=[]):
    ...
    not_voc_ocu = [x for x in text_ocupation.split() if x not in voc_ocu]
    
    not_voc_act = [x for x in text_activity.split() if x not in voc_act]
    
    if len(not_voc_ocu) > 0:
        not_voc_ocu.insert(0,"No aparecen en vocabulario de entrenamiento de ocupación:")
        flag += " ".join(not_voc_ocu)
        
    if len(not_voc_act) > 0:
        not_voc_act.insert(0," No aparecen en vocabulario de entrenamiento de actividad:")
        flag += " ".join(not_voc_act)
    
    
    text_ocupation = tokenizer_steam_words(text_ocupation)
    ...
    ocupation_result = occupation_predict_coding(text_ocupation,activity_code,F73_code)
    
    pred_ocu_prob =  ocupation_result[2]
    pred_F71_2 = ocupation_result[0]
    ...
    return ocupation_result, activity_result, flag

def process_string_to_predict(string):
    """Process strings for prediction""" 
    stop = get_stop_words()
    try:
        steamed = []
        tokens = tokenizer_words(string.strip())
        stemer = SpanishStemmer()
        if len(tokens) > 1:
            tokens = [x for x in tokens if x.lower() not in stop]
        for token in tokens:
            steamed.append(stemer.stem(token))
...
def validate_models():
    ...
    success_counter_ocupation = 0
    success_counter_activity = 0
    success_counter_ocupation_3rd = 0
    success_counter_activity_3rd = 0
    success_counter_ocupation_2nd = 0
    success_counter_activity_2nd = 0
    pred = pd.DataFrame()
    df = pd.read_excel(path_validation_data,dtype={'F72_2':str,'F71_2':str},engine='openpyxl')  

    counter = 0
    for index, row in df.iterrows():
        counter += 1
        ocupation_text = row['F71_1']
        ...
        try:
            ocupation_result, activity_result, flag = predict_code(ocupation_text,activity_text,F73_code)
        except Exception as e:
            print(e)
        ...
        pred_F71_2 = ocupation_result[0]
        pred_ocu_cat =  ocupation_result[1]
        pred_ocu_prob = ocupation_result[2]
        ...
        if counter % 1000 == 0 and counter > 0:
            ...
            acc_por_tot_ocu =  success_counter_ocupation/counter*100
            acc_por_2nd_ocu = success_counter_ocupation_2nd/counter*100
            acc_por_2nd_act = success_counter_activity_2nd/counter*100
            acc_por_3rd_ocu = success_counter_ocupation_3rd/counter*100
            acc_por_3rd_act = success_counter_activity_3rd/counter*100
            
            
            print('Van {} predicciones, con {} de aciertos totales, un {:.2f} %'.format(counter,success_counter_all,success_counter_all/counter*100))
            print('Van {} predicciones, con {} de aciertos de actividad, un {:.2f}  , {:.2f}% al tercer nivel, {:.2f} % a la segundo nivel'.format(counter,success_counter_activity,acc_por_tot_act,acc_por_3rd_act,acc_por_2nd_act))
            print('Van {} predicciones, con {} de aciertos de ocupacion, un {:.2f} %, {:.2f}% al tercer nivel, {:.2f} % a la segundo nivel'.format(counter,success_counter_activity,acc_por_tot_ocu,acc_por_3rd_ocu,acc_por_2nd_ocu))
