"""This module creates the training and validation datasets from an input file"""

import os
import pandas as pd


def create_occupation_training_dataset(train):
    """Creates dataframe for training models for occupation"""
    ocupa = train.copy()
    ...
    ocupa.rename(
        {"F71_1": "TEXTO_OCUPACION", "F72_2": "CODIGO ACTIVIDAD"}, axis=1, inplace=True
    )
...
def create_activation_training_dataset(train):
    ...
    acti.rename(
        {
            "F72_1": "TEXTO_ACTIVIDAD",
            "F72_2": "CODIGO ACTIVIDAD",
        },
        axis=1,
        inplace=True,
    )
...
def get_relative_path(relative_path):
    """Gets the relative path from the file"""
...
def create_dataframes(path=".\data\Datos depurados a mano.xlsx", val_size=10000):
    ...
    create_occupation_training_dataset(train)
...
if __name__ == "__main__":
    ...
    create_dataframes(ORIGINAL_FILE)
