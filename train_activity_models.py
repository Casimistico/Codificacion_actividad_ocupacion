...
def train_best_model(df_train,target_layer,column_text,index):
    ...
    parameters = pd.read_excel(".\modelos_actividad\\parametros.xlsx",engine='openpyxl')
    ...
    vect = CountVectorizer(max_df=vect__max_df, ngram_range=ngram_range,max_features=vect__max_features).fit(X_train)
    ...
    model = SGDClassifier(alpha=clf__alpha, max_iter=150, penalty=clf__penalty, tol=1e-3)
...
def train_model_smote(df_train,target_layer,column_text):
    ...
    y_train = pd.concat([y_train,y_train2,y_train3,y_train4,y_train5,y_train6,y_train7])
    ...
    X_train = pd.concat([X_train,X_train2,X_train3,X_train4,X_train5,X_train6,X_train7])
    ...
    smote = SMOTE()
    X_train_vectorized_smote, y_train_smote = smote.fit_sample(X_train_vectorized,y_train)
...
def train_base_layer_classifier(df):
    ...
    trained_model, vect, acc, f1 = train_best_model(df,'Primera','TEXTO_ACTIVIDAD',"Base")
...
def train_first_layer_classifier(df):
    ...
    for first in range(10):
        ...
        if len(select)>=1:
            ...
            trained_model, vect, acc, f1 = train_best_model(select,'Segunda','TEXTO_ACTIVIDAD',index)
    ...
    for first in range(10):
        for second in range(10):
            ...
            if len(select['Tercera'].unique())>1:
                ...
                trained_model_smote, vect_smote, acc_smote, f1_smote = train_model_smote(select,'Tercera','TEXTO_ACTIVIDAD')
                trained_model_simple, vect_simple, acc_simple, f1_simple = train_best_model(select,'Tercera','TEXTO_ACTIVIDAD',index)
        ...
        for second in range(10):
            for third in range(10):
                ...
                if len(select['Cuarta'].unique())>1:
                    print("Primera {}, segunda {}, tercera {}".format(first,second,third))
                    ...
                    trained_model_smote, vect_smote, acc_smote, f1_smote = train_model_smote(select,'Cuarta','TEXTO_ACTIVIDAD')
                    trained_model_simple, vect_simple, acc_simple, f1_simple = train_best_model(select,'Cuarta','TEXTO_ACTIVIDAD',index)
                    if acc_smote > acc_simple:
                        print("Se usa SMOTE, {:.3f} simple, {:.3f} SMOTE".format(acc_simple,acc_smote))
                        trained_model = trained_model_smote
                        vect = vect_smote
                        acc = acc_smote
                        f1 = f1_smote
